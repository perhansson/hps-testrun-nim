...
half of the ECal. The trigger system can be broken down into three sections (see 
...
programmable with 4~ns resolution. 
...
\begin{center}
{\small
    	\includegraphics[width=7cmh]{figures/single_hit_efficiency_Omar_11192013.pdf}
...
subtracting the pedestal then applying a gain or conversion factor to convert ADC counts to energy. 
The pedestals was measured in special runs where a 100 samples long window was recorded for each 
trigger in which the FADCs sampled the APD signals every 4~ns.  The pedestals were extracted from the 
part of the window before the actual hit in the calorimeter. Modules above a threshold are clustered 
using a simple algorithm described in Sec.~\ref{sec:trigger}. The ECal was operated with an effective 
crystal readout threshold of 73~MeV. The noise level of the ECal modules were measured to be approximately 15~MeV. 

To compare data and simulation, all inoperable or noisy channels in SVT and ECal have been disabled 
in the simulation; any efficiency or bias effects that affect the real data should be reflected 
in the simulation. Furthermore, a gain calibration is performed to equalize the gains in the 
simulation to those observed in data by comparing the ECal cluster energy $E$ to the momentum $p$ of 
matched tracks in the SVT.  The ``weighted $E/p$'' for each crystal is computed, representing the 
average $E/p$ for clusters that include the crystal
% $\frac{\sum_j w_{j,i}}{\sum_j\frac{P_j}{E_j}w_{j,i}}$.
Iteratively, gains are adjusted until the weighted $E/p$ is similar in data and simulation. The $E/p$ 
in data and simulation can be seen in Fig.~\ref{fig:gains}.
\begin{figure}[]
\begin{center}
{\small
	\includegraphics[width=7cm]{figures/eoverp_MC_egs_0_016x0_June5_2013_Sho_Ecal_presentation.png}
	\includegraphics[width=7cm]{figures/eoverp_data_run1351_June5_2013_Sho_Ecal_presentation.png}
	\caption{The ECal energy over track momentum ratio for simulation (top) and data (bottom).\color{red}{\bf (Update style.)}} 
	\label{fig:gains}
}
\end{center}
\end{figure}
These gains can then be used to convert from the ADC counts reported by the FADC in each channels 
to the energy deposited in the crystals. In order to find the energy of the incident particle one also needs 
to know the  the fraction of incident energy that is deposited in the cluster or "sampling fraction". 
The sampling fraction is parameterized from simulation that show that it is approximately 0.9 for the 
ECal. The weighted $E/p$ shown in Fig.~\ref{fig:gains} is an approximate measurement of the sampling 
fraction of that part of the ECal but is position- (somewhat smaller at the edges) and energy-dependent 
because of the high readout thresholds (explaining the low value compared to 0.9). Since the 
sampling fraction is not needed to normalize event rates it was not applied in the multiple scattering 
analysis in Sec.~\ref{sec:mcs}.
...
but based on comparisons with simulation we believe the trigger worked exactly as specified.
\begin{figure}[ht]
\begin{center}
{\small
...
\end{center}
\end{figure}
The trigger turn-on is slow and reaches an uneven plateau just below one for two reasons.  Gain 
variations between different crystals lead to threshold variations and the nonlinearity of the 
...
as well as cluster multiplicity. Overall the trigger appears to have functioned exactly as intended. 
