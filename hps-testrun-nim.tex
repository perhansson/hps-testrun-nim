...
subtracting the pedestal then applying a conversion factor to convert ADC counts to energy. 
The pedestals are measured using special runs where a 100 samples of APD signals, $4$ ns a part, have been recorded at each trigger.  The pedestals were extracted from the 
part of the $400$ ns window before the actual hit in the calorimeter. After subtracting the pedestal, modules with integrated pulse above a threshold are clustered 
using a simple algorithm similar to the one deployed for the trigger, see Sec.~\ref{sec:trigger}. Due to a high readout threshold of $73$~MeV, the average number of modules in a cluster was $\sim 3$ and the simple clustering algorithm worked well for reconstruction of detected shower energy. From pedestal runs the average noise level of the ECal modules were measured to be approximately 15~MeV. 

In order to determine conversion factors from ADC counts to energy, the ratios the ECal cluster energy $E$ to the momentum $p$ of  matched tracks in the SVT have been studied in the data and simulation. To compare data and simulation, all inoperable or noisy channels in SVT and ECal have been disabled 
in both data and the simulation. Any efficiency or bias effects that affect the real data should be reflected 
in the simulation. 
%Furthermore, a conversion factor calibration is performed to equalize the gains of  each channel.  simulation to those observed in data by comparing .  The ``weighted $E/p$'' for each crystal is computed, representing the average $E/p$ for clusters that include the crystal
% $\frac{\sum_j w_{j,i}}{\sum_j\frac{P_j}{E_j}w_{j,i}}$.
Iteratively, conversion coefficients have been adjusted until the $E/p$ ratios in the data became similar to the  ones in simulation. The distribution of the $E/p$ ratio in data and simulation can be seen in Fig.~\ref{fig:gains}. The peak position of the distribution shows the sampling fraction of the ECal, the fraction of the incident energy detected in the cluster. The width and tails of the distribution in data indicates imperfect calibration and noise of ECal modules. This level of calibration and the agreement with simulation was found to be sufficient for the multiple scattering analysis using normalized event rates, see Sec.~\ref{sec:mcs}.
\begin{figure}[]
\begin{center}
{\small
