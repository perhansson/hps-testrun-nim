...
detectors down to 15~mrad opening angle only 10~cm behind the target. HPS employs a $\approx 1$~m long 
silicon tracking and vertexing detector
inside a dipole magnet to measure charged particle trajectories and find the decay vertex position of oppositely 
charged pairs. A fast electromagnetic calorimeter sits just downstream of the magnet to provide a trigger and 
identify electrons. To overcome the 
...
front-end electronics. HPS is designed to run at  high trigger and data throughput rates in order to exploit the 
499~MHz bunch structure,  100\% duty 
...
experiment, where studied. This dedicated running provided valuable data to demonstrate the functionality of 
the apparatus, document its performance, and explore trigger rates, as shown below. 
...
Good mass resolution is needed across the interesting mass range to discover the \Aprime{} 
signal on top of the copious QED background in the bump-hunt search. Track momentum and angular 
resolution of 4-5\% and 1-4~mrad (across the momentum range in the non-bend 
plane which dominates the measurement), respectively, translates into a 2.5\% mass resolution for \Aprime{} 
decay opening angles between 15-70~mrad~\cite{HPS_proposal_2010}. With multiple scattering dominating 
both mass and vertexing uncertainties, spatial hit 
...
While the trigger for \Aprime{} events is largely insensitive to the energy resolution, in order to have the 
...
top and bottom halves of the ECal satisfying loose kinematic selections optimized on simulated \Aprime{} 
events to further reduce the rate. The trigger system need to be essentially dead time free, handle rates up to 
