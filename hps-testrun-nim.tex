...
magnet. Data was taken on three different converter thicknesses with a beam current varying between 
...
\begin{tabular}{|c|c|c|}
\hline
Converter thickn. & Duration &  $e^-$ on converter \\
 (\%$X_0$) & (s) & ($\mu$C)    \\   
\hline
%\hline
...
The runs taken during the dedicated HPS Test run is summarized in Tab.~\ref{tab:currents}.
...
the characteristic Landau shape, see Fig.~\ref{fig:cluster_pulse}.  
...
target was located approximately 67~cm upstream from our nominal target position; giving almost 
collinear tracks in the detector. This degrades the vertex resolution along the 
beam line compared to that expected in an electron beam with nominal target position. 
...
\subsection{Multiple Coulomb Scattering Distributions}
...
Despite all data taken with a photon beam in the Test run, the angular distribution of the pair 
produced electron and positron emerging from the converter with different converter thickness can 
be used to study the varying multiple Coulomb scattering contribution (pair production angle is constant). 
Restricting clusters to a well calibrated region of the ECal we observe, within our systematic 
uncertainties , good agreement with the rates predicted by \egs{}, see Tab.~\ref{tab:ang_distr_dataMC}.
%\begin{center}
\begin{table}
{\small
...
As described in Sec.~\ref{sec:fadc} and shown in Fig.~\ref{fig:hps_trigger_data} the energy from each 
crystals are measured differently in the trigger and what is readout from the ECal. 
