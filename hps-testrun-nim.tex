...
\label{introduction}
The heavy photon (\Aprime{}), aka a ``hidden sector'' or ``dark'' photon, is a massive particle which 
...
couplings, the \Aprime{} become long-lived, so detection of a displaced decay vertex can reject the prompt 
QED background and boost experimental sensitivity.  

The HPS experiment~\cite{HPS_proposal_2010} is preparing for installation in Hall-B at JLab in the Fall 
of 2014. It searches for electro-produced \Aprime{} in interactions between electrons and nuclei in a thin 
(0.25\% R.L.) Tungsten target foil. The electrons are supplied by the nearly continuous JLab CEBAF 
beam with energy between 2.2 and 6.6~GeV. The HPS experiment uses both 
invariant mass and secondary vertex signatures to search for \Aprime{} decaying into \ee{} pairs. In the 
interesting mass and coupling range, the decay products are boosted along the beam axis with small 
opening angles and decay lengths ranging from mm's to tens of cm's requiring HPS to operate tracking 
detectors down to 15~mrad opening angle only 10~cm behind the target. It employs a $\approx 1$~m long 
silicon tracking and vertexing detector
inside a dipole magnet to measure and vertex charged particle trajectories and a fast electromagnetic 
calorimeter just downstream of the magnet to provide a trigger and identify electrons. To overcome the 
background occupancy from multiple scattered beam electrons both detectors have excellent timing 
resolution and the experiment utilizes very high-rate 
front-end electronics and runs at high trigger rates, exploiting the 499~MHz bunch structure,  100\% duty 
...
\begin{itemize}
\item uniform acceptance between 15 and $\approx$70~mrad in the forward region to catch boosted decay products close to the beam, 
...
\item high-rate electronics with trigger rates up to 50~kHz and data handling rates of 100~MB/s to permanent storage, 
...
\item excellent hit time resolution to reject out-of-time background hits in the nearly continuous CEBAF 
beam; 2~ns for hits on tracks in a 8~ns trigger window, 
\item momentum and angular resolution better than 4.5\% and 1 to 4~mrad (across the track momentum 
range in non-bend direction), respectively, to reach \Aprime{} 
mass resolution of 2.5\% or less across the interesting mass range, 
\item resolution of distance of closest approach to the beam axis less than 250 (100)~$\mu$m for tracks with 
0.5 (1.7)~GeV/c momentum to achieve a Gaussian vertex resolution down to 1~mm at high masses to 
discriminate displaced \Aprime{} decays from prompt QED backgrounds,
\end{itemize}
...
precluded any dedicated electron beam running, but the HPS Test Run was allowed a short ($\approx$6h) 
and valuable dedicated photon beam run at the end of scheduled CEBAF running. During this dedicated 
period, \ee{} pairs, produced in a gold foil upstream of the 
experiment, where studied. With no dedicated electron beam running no search for \Aprime{} signal was 
possible but this final running provided enough data to demonstrate the functionality of the 
apparatus, document its performance, and explore trigger rates, as shown below. 
...
to, the level required for calculating the physics reach of the HPS experiment. 
...
The \Aprime{} decay products have momenta in the range of 0.4-2.0~GeV$/$c, so multiple scattering 
...
The mass resolution need to be around 2.5\% across the interesting mass range to discover the \Aprime{} 
signal on top of the copious QED background in the bump-hunt search. With \Aprime{} decay opening 
angles between 15-70~mrad this translates into track momentum and angular resolution of 4-5\% and 
1-4~mrad (across the momentum range in the non-bend plane which dominates the measurement)
~\cite{HPS_proposal_2010}. With multiple scattering dominating both mass and vertexing uncertainties, spatial hit 
resolution less than 50~$\mu$m are required in the non-bend plane in the first few layers which dominates 
the vertexing performance and about twice that in the non-bend plane. 
...
readout speed to handle 1~MHz/cm$^{2}$ of electromagnetic background and to be radiation hard. 
While the signal \Aprime{} trigger is largely insensitive to the energy resolution, in order to have the 
possibility to improve the energy resolution from the SVT measurement  
$\sigma(E)$/$E$$<$4.5\%/$\sqrt(E)$ is needed. 
...
results in about 8 photoelectrons/MeV which needs to be amplified before fed into the FADC for digitization and 
...
FADC. With a total noise level of about 10~MeV and a resolution of about 1~ADC/MeV the ADC resolution 
fulfills the requirements from HPS.
...
top and bottom halves of the ECal satisfying loose kinematic selections optimized on \Aprime{} events 
to further reduce the rate. The trigger system need to be essentially dead time free, handle rates up to 
50~kHz and importantly supply a trigger time relative to the beam bunches with 8~ns to reduce the 
background from out-of-time hits in the high occupancy regions of the SVT.  Figure~\ref{fig:hps_trigger_cal} 
shows a schematic overview of each stage of the system. 
 \begin{figure}[b]
\begin{center}
...
each of the 20 silicon strip sensors of the SVT. The data for each strip channel, six analog samples of the 
...
allowed the study of some of the key performance parameters for HPS and also explore the expected 
trigger rates during electron beam running. This section documents the performance and discusses the 
results implication for HPS.
...
Lab-based radioactive source tests were used to 
...
of 4-5\% is needed to reach the expected \Aprime{} mass resolution. The precise reconstruction of the 
...
of 73~MeV the average number of crystals in a cluster was $\sim 3$ and the simple clustering 
...
would be adequate. However, improvements are needed to achieve the expected energy resolution in HPS ($<4.5\%/\sqrt{E}$). The noise and and thresholds need to be closer to 10~MeV the 
thresholds will  be lowered and a more elaborate calibration technique needs to be employed. In addition the number of 
working channels needs to be improved by equipping the detector new and complete electronics for each 
channel.
